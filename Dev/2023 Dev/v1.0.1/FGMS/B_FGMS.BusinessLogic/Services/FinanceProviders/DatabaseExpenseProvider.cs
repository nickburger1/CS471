...
namespace B_FGMS.BusinessLogic.Services.FinanceProviders
{
    ...
    public class DatabaseExpenseProvider : IExpenseProvider
    {
        ...
        /// <summary>
        /// This method will return true if a donation typeitem is used
        /// </summary>
        /// <param name="tuid"></param>
        /// <returns></returns>
        /// <author>Andrew Loesel</author>
        /// <created>4/13/2023</created>
        /// <note>This method is using the same error codes as GetExpenseByExpenseTypeId</note>
        public bool DonationTypeUsed(int tuid)
        {
            try
            {
                var type = _dbContext.DonationTypeItems.Where(x => x.Tuid == tuid).FirstOrDefault();
                if(type != null)
                {
                    var item = _dbContext.InKindDonationTypeItems.Where(x => x.DonationTypeItemTuid == tuid).FirstOrDefault();
                    if(item != null)
                    {
                        return true;
                    }
        ...
        /// <summary>
        /// This method will add a new donation type
        /// </summary>
        /// <param name="name"></param>
        /// <author>Andrew Loesel</author>
        /// <created>4/13/2023</created>
        /// <note>This method shares error codes with the AddNewExpenseType function</note>
        public void AddNewDonationType(string name)
        {
            try
            {
                var existingItem = _dbContext.DonationTypeItems.Where(x => x.Name== null ? false : x.Name.Equals(name)).FirstOrDefault();
                if(existingItem == null)
                {
                    var newItem = new DonationTypeItem
                    {
                        Name = name,
                        Description=""
                    };
                    _dbContext.DonationTypeItems.Add(newItem);
                    _dbContext.SaveChanges();
                }
            }
            catch (SqlException e)
        ...
        /// <summary>
        /// This method will delete the given donation type
        /// </summary>
        /// <param name="item"></param>
        /// <author>Andrew Loesel</author>
        /// <Created>4/13/2023</Created>
        /// <note>This method uses the same error codes as DelExpenseType</note>
        public void DeleteDonationType(ExpenseTypeModel item)
        {
            try
            {
                var existingItem = _dbContext.DonationTypeItems.FirstOrDefault(x => x.Tuid.Equals(item.Tuid));
                if(existingItem != null)
                {
                    _dbContext.DonationTypeItems.Remove(existingItem);
                    _dbContext.SaveChanges();
                }
            }
            catch (SqlException e)
        ...
        public bool AddExpense(DateTime date, double dblValue, int intExpenseTypeTuid, int intVolunteerTuid)
        {
            try
            {
                ExpenseTypeItem? type = _dbContext.ExpenseTypeItems.Where(x => x.Tuid == intExpenseTypeTuid).FirstOrDefault();
                if (type != null)
                {
