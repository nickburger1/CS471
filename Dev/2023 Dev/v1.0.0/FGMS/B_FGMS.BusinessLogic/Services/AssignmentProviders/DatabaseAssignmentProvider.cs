 ...
 **/
    public class DatabaseAssignmentProvider : IAssignmentProvider
    {
        ...
        public int GetActiveVolunteersBySchoolAssignmentCount(int schoolTuid)
        {
            try 
            { 
                return _dbContext.Assignments.Include(x => x.Classroom).Where(x => x.Classroom.SchoolTuid == schoolTuid && x.Classroom.IsDeleted == false).Include(x => x.Volunteer).Where(x => x.Volunteer.SeparatedDate == null && x.Volunteer.IsDeleted == false).Count();
            }
        ...
        public IEnumerable<Assignment> GetAllAssignments()
        {
            try
            {
                return _dbContext.Assignments.Where(x => x.IsDeleted != false).Include(x => x.Classroom).Include(x => x.Classroom.School).Include(x => x.Volunteer).ToList();
            }
            catch (SqlException e)
            {
                if (e.ErrorCode == -2146232060)
